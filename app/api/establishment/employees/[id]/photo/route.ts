/**
 * POST /api/establishment/employees/[id]/photo — загрузка фото сотрудника (профиль или для печати).
 * FormData: file (обязательно), type = "avatar" | "print".
 * avatar — для страницы оплаты и ЛК (рекомендуется ≥200×200 px).
 * print — только для карточки печати (рекомендуется ≥150×150 px).
  ...
  try {
    ...
    if (!type || !ALLOWED_TYPES.includes(type as (typeof ALLOWED_TYPES)[0])) {
      return NextResponse.json(
        { error: 'Укажите type: "avatar" (страница оплаты и ЛК) или "print" (карточка для печати)' },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      logError("employee.photo.upload.write.error", err, { employeeId, type });
      return NextResponse.json(
    ...
    logError("employee.photo.upload.error", err, {});
