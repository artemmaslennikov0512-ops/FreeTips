  ...
  return (
    <div className="flex min-w-0 max-w-full flex-col gap-6">
      ...
      <form
        ...
        className="flex flex-col gap-3 rounded-[10px] border border-[var(--color-brand-gold)]/30 bg-white/[0.06] p-4 sm:flex-row sm:flex-wrap sm:items-end"
      >
        <div className="min-w-[12rem] flex-1">
          ...
          <select
            ...
            className="w-full rounded-lg border border-[var(--color-brand-gold)]/25 bg-[#0a192f]/30 px-3 py-2 text-[var(--color-on-dark)]"
          >
        ...
        <>
          <section>
            ...
            {openSessions.length === 0 ? (
              <p className="text-sm text-[var(--color-on-dark-muted)]">Нет открытых сессий.</p>
            ) : (
              <ul className="flex flex-col gap-2">
                {openSessions.map((s) => (
                  <li
                    key={s.id}
                    className="flex flex-wrap items-center justify-between gap-2 rounded-[10px] border border-[var(--color-brand-gold)]/25 bg-white/[0.06] p-4"
                  >
                    <div>
                      <p className="font-medium text-[var(--color-on-dark)]">{s.tableLabel}</p>
                    ...
                    <button
                      ...
                      className="rounded-xl border border-[var(--color-brand-gold)]/40 px-3 py-1.5 text-sm text-[var(--color-on-dark)] hover:bg-white/10"
                    >
                      Закрыть
                    </button>
                  </li>
          ...
          <section>
            <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-[var(--color-on-dark-muted)]">
              Недавние (все статусы)
            </h2>
            <ul className="flex max-h-[24rem] flex-col gap-1 overflow-y-auto text-sm">
              {closedSessions.slice(0, 50).map((s) => (
                <li key={s.id} className="rounded-lg border border-[var(--color-brand-gold)]/15 bg-white/[0.04] px-3 py-2 text-[var(--color-on-dark-muted)]">
