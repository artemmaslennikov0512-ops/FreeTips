...
/** Для заголовков/API: только валидный UUID v4, иначе null. */
export function normalizeDeviceClientId(raw: string | null | undefined): string | null {
  const v = raw?.trim();
  if (!v || !isDeviceClientIdUuidV4(v)) return null;
  return v;
}
...
export function getOrCreateDeviceClientId(): string {
  ...
  try {
    ...
    if (existing && isDeviceClientIdUuidV4(existing)) return existing;
