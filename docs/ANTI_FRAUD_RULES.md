...
**Текущее состояние:** в `app/api/pay/[slug]/route.ts` включены лимиты по IP и по slug (см. `lib/middleware/rate-limit.ts`).
...
| Верхняя граница схемы (технический потолок Zod) | 100 млн ₽ | Защита от переполнения; бизнес-лимит ниже |
| Лимит на один платёж (продукт) | **1 000 ₽ (100 000 коп)** | Снизить риск крупных краденых карт; единый лимит на UI и API |
...
2. Минимальная сумма платежа 1 ₽, максимум за один платёж 1 000 ₽.
...
- `lib/payment-amount-bounds.ts` — минимум/максимум суммы одного платежа (чаевых).
- `lib/validations.ts` — схема `createPaymentSchema` и прочие лимиты.
