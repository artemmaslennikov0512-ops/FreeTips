...
**Сборка в Docker:** в `Dockerfile` этап `builder` вызывает `npm run build` (единый контракт с локальной разработкой и CI: `prisma generate` + Next с флагом `--webpack`). Перечень npm-скриптов и порядок проверок в GitHub Actions — в [README.md](./README.md) (раздел «Сборка и проверки»).
