...
const API_RATE_DEFAULT_MAX =
  typeof process !== "undefined" && process.env.NODE_ENV === "production" ? 2400 : 2000;
const API_RATE_LIMIT_MAX =
  typeof process !== "undefined" && process.env.API_RATE_LIMIT_MAX
    ? Math.max(200, parseInt(process.env.API_RATE_LIMIT_MAX, 10) || 2400)
    : API_RATE_DEFAULT_MAX;
const API_RATE_API_KEY_DEFAULT_MAX =
  typeof process !== "undefined" && process.env.NODE_ENV === "production" ? 30000 : 30000;
const API_RATE_LIMIT_API_KEY_MAX =
  typeof process !== "undefined" && process.env.API_RATE_LIMIT_API_KEY_MAX
    ? Math.max(2000, parseInt(process.env.API_RATE_LIMIT_API_KEY_MAX, 10) || 30000)
...
function buildCsp(isDev: boolean): string {
  ...
  if (!isDev) lines.push("upgrade-insecure-requests");
  return lines.join("; ");
}
...
export const config = {
  matcher: ["/((?!_next/static|_next/image|favicon\\.ico|favicon\\.png|icon\\.svg|sitemap\\.xml|robots\\.txt).*)"],
};
