...
export function TestLkMockShellThemeToggle({ compact }: { compact?: boolean }) {
  const { shellTheme, setShellTheme } = useTestLkMockShellTheme();
  return (
    <div className={`flex items-center gap-1 ${compact ? "" : "rounded-lg border p-0.5"}`} style={{ borderColor: "var(--tlk-border)" }}>
