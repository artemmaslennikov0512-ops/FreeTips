/**
 * Нагрузочный тест (симуляция вебхуков): 100 тестовых ЛК, по 20 заявок на аккаунт.
 * Платежи создаются через createPayment(); «оплата» — отправкой имитированных вебхуков Paygine.
 * Окно оплаты не открывается, номер карты не вводится.
 *
 * Для теста с реальным вводом карты на форме Paygine используйте:
 *   npx tsx scripts/load-test-payments-e2e.ts [N]
 *
 * Требуется:
