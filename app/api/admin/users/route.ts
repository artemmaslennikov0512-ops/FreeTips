...
export async function GET(request: NextRequest) {
  ...
  const txMap = new Map(
    txAgg.map((t) => [t.recipientId, { sum: t._sum.amountKop ?? BigInt(0), count: t._count._all }]),
  );
  const pendingMap = new Map(payoutPendingAgg.map((p) => [p.userId, p._count._all]));
  const completedMap = new Map(
    payoutCompletedAgg.map((p) => [p.userId, p._sum.amountKop ?? BigInt(0)]),
  );

  return NextResponse.json({
    users: users.map((u) => {
      const tx = txMap.get(u.id) ?? { sum: BigInt(0), count: 0 };
      const pendingCount = pendingMap.get(u.id) ?? 0;
      const completedSum = completedMap.get(u.id) ?? BigInt(0);
      const balance = tx.sum - completedSum;
      return {
        id: u.id,
        uniqueId: u.uniqueId,
        login: u.login,
        ...
        stats: {
          ...
          totalReceivedKop: Number(tx.sum),
