...
- **Сборка:** локально и в Docker единый вход `npm run build` (`prisma generate` + `next build --webpack`); CI в `.github/workflows/ci.yml` — две стадии (`ci:checks` без БД, затем сборка и тесты с Postgres). См. README, раздел «Сборка и проверки».
