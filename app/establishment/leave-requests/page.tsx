...
export default function EstablishmentLeaveRequestsPage() {
  ...
  const load = useCallback(async () => {
    setError(null);
    const res = await fetchWithAuth("/api/establishment/leave-requests?status=PENDING");
    if (!res.ok) {
  ...
  const approve = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetchWithAuth(`/api/establishment/leave-requests/${id}/approve`, {
        method: "POST",
  ...
  const reject = async (id: string) => {
    const reason = window.prompt("Причина отклонения (необязательно). Официант останется в заведении.") ?? "";
    setActingId(id);
    setError(null);
    try {
      const res = await fetchWithAuth(`/api/establishment/leave-requests/${id}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4 pb-2">
      ...
      {rows.length === 0 ? (
        <p className="text-center text-sm text-white/70">Нет ожидающих заявок на выход.</p>
      ) : (
