...
- **Текущие эндпоинты** доступны по путям вида `/api/...` (без явного номера версии в URL).
- **Стабильный контракт:** при появлении несовместимых изменений новые версии будут выноситься под префикс, например `/api/v2/...`. Текущие пути сохраняются как фактическая «v1» до объявления устаревания.
- **Обратная совместимость:** в рамках одной мажорной версии не удаляются и не переименовываются существующие поля и эндпоинты; новые опциональные поля допускаются. О планируемых breaking changes будет объявляться заранее (changelog, документация).
- **Внешние интеграции** (вебхуки, мобильное приложение) при необходимости могут опираться на явный префикс версии, когда он будет введён.
