...
### Нагрузочные тесты (чаевые)

| Скрипт | Назначение |
|--------|------------|
| **load-test-payments.ts** | Создаёт 100 пользователей, по 20 платежей на каждого; «оплата» — отправкой **имитированных вебхуков**. Окно оплаты не открывается, карта не вводится. |
| **load-test-payments-e2e.ts** | **Полноценный сценарий под нагрузкой**: страница оплаты → Paygine → ввод карты → оплата → вебхук → **перелив** (SDRelocateFunds) на кубышку официанта. По БД выводится, сколько транзакций в SUCCESS (перелив выполнен), сколько в PENDING. |

Полноценный E2E с переливом (как ведёт себя сайт под нагрузкой):

```bash
npx tsx scripts/load-test-payments-e2e.ts [число_платежей] [параллельных_потоков]
```

По умолчанию: 20 платежей, 1 поток. Пример с нагрузкой: `50 3` — 50 оплат в 3 параллельных потока. Нужны приложение (BASE_URL, вебхуки должны доходить), DATABASE_URL, Paygine и тестовая карта в scripts/.env. `HEADLESS=0` — показать браузер.

**Закрытие 2k заявок от load-test-payments:** заявки в PENDING с `idempotencyKey` вида `loadtest-*` можно «дожать» вебхуками или пометить как успешные для очистки:
- `npx tsx scripts/load-test-complete-pending.ts` — отправить вебхуки; приложение обработает их, перелив в Paygine не пройдёт (заказы не оплачены картой), заявки перейдут в **FAILED**.
- `npx tsx scripts/load-test-complete-pending.ts --mark-success` — пометить все такие PENDING как **SUCCESS** в БД (тестовая очистка, без вызова Paygine).
