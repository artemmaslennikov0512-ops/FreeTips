...
export function templatePasswordReset(params: { resetLink: string }) {
  ...
  const html = `
    ...
    <p><a href="${params.resetLink}">Перейти к сбросу пароля</a></p>
...
export function templateRegistrationLinkFromRequest(params: {
  ...
  const html = `
    <p>Здравствуйте${params.fullName ? `, ${params.fullName}` : ""}!</p>
    <p>Ваша заявка на подключение к сервису чаевых FreeTips одобрена.</p>
    <p><a href="${params.link}">Перейти к регистрации</a></p>
...
export function templateInviteEmployee(params: {
  ...
  const html = `
    <p>Здравствуйте!</p>
    <p>Вам направлена ссылка для регистрации в сервисе чаевых FreeTips (как официант${params.employeeName ? ` — ${params.employeeName}` : ""}).</p>
    <p><a href="${params.link}">Перейти к регистрации</a></p>
