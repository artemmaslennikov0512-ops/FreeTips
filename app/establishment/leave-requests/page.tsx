...
export default function EstablishmentLeaveRequestsPage() {
  ...
  const load = useCallback(async () => {
    setError(null);
    const res = await fetch("/api/establishment/leave-requests?status=PENDING", { headers: authHeaders() });
    if (!res.ok) {
  ...
  const approve = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/establishment/leave-requests/${id}/approve`, {
        method: "POST",
  ...
  const reject = async (id: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/establishment/leave-requests/${id}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders(), ...getCsrfHeader() },
