 ...
 * FormData: requestId, type (passport_spread | selfie), file.
...
const ALLOWED_TYPES = ["passport_spread", "selfie"] as const;
  ...
  try {
    ...
    if (!requestId || !type || !ALLOWED_TYPES.includes(type as (typeof ALLOWED_TYPES)[0])) {
      return NextResponse.json(
        { error: "Укажите requestId и type (passport_spread или selfie)" },
        { status: 400 },
      );
    }
